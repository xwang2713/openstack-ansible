...
apt-get install -y python-dev libv8-dev openjdk-6-jdk openjdk-7-jdk libapr1-dev libaprutil1-dev libiberty-dev
...
then
  ...
  #cd jenkins
  #scp -o StrictHostKeyChecking=no root@1.0.0.17:/data3/software/jenkins/*.jar .
  #cd ..
...
then
   cd /Downloads
   wget http://1.0.0.17:/data3/software/cmake/cmake-3.5.2-trusty-amd64.tar.gz
