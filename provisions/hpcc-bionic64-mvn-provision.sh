...
apt-get install -y libv8-dev openjdk-8-jdk libapr1-dev libaprutil1-dev libiberty-dev
...
if [ ! -d $ant_name ]
then 
  scp -o StrictHostKeyChecking=no root@${FILE_SERVER}:/data3/software/apache/${ant_name}-bin.tar.gz .
  tar -zxvf ${ant_name}-bin.tar.gz
  rm -rf ${ant_name}-bin.tar.gz
  [ -d ant ] && rm -rf ant
  ln -s $ant_name ant
  grep ANT_HOME /etc/profile 
  if [ $? -ne 0 ]
  then
