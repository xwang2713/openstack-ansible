...
#-------------------------------
apt-get update
apt-get install -y libgtk2.0-dev curl libcurl4-gnutls-dev fop zip 
...
cd /Downloads
scp -o StrictHostKeyChecking=no root@${FILE_SERVER}:/data3/software/R/RInside_0.2.10.tar.gz .
...
then
   cd /Downloads
   wget http://${FILE_SERVER}:/data3/software/cmake/cmake-3.5.2-precise-i686.tar.gz
   tar -zxf cmake-3.5.2-precise-i686.tar.gz
   rm -rf  cmake-3.5.2-precise-i686.tar.gz
